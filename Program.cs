...
namespace Stock_Data_Processing
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            CustomerRecords customer = JsonConverter.DeserializeObject<CustomerRecords>(File.ReadAllText(@"G:\net\Stock_Data_Processing\Stock_Data_Processing\CustomerRecords.json"));
            StockAccount stockAccount = JsonConvert.DeserializeObject<StockAccount>(File.ReadAllText(@"G:\net\Stock_Data_Processing\Stock_Data_Processing\StockAccount.json"));
            Transaction transaction = JsonConvert.DeserializeObject<Transaction>(File.ReadAllText(@"G:\net\Stock_Data_Processing\Stock_Data_Processing\Transaction.json"));
            while (!validation)
            {
                try
                {
                                        ...
                                        foreach (StockAccount.Records i in stockAccount.Company_Records)
                                        {
                                            if (i.Symbol.Equals(shareName))
                                            {
                                                ...
                                                if (stockNo > 0 && stockNo < record.StockDetails[shareName])
                                                {
                                                    ...
                                                    File.WriteAllText(@"G:\net\Stock_Data_Processing\Stock_Data_Processing\CustomerRecords.json", JsonConverter.SerializeObject(customer));
                                                    File.WriteAllText(@"G:\net\Stock_Data_Processing\Stock_Data_Processing\StockAccount.json", JsonConverter.SerializeObject(stockAccount));
                                                    File.WriteAllText(@"G:\net\Stock_Data_Processing\Stock_Data_Processing\Transaction.json", JsonConverter.SerializeObject(transaction));
                                    ...
                                    BalanceValid:
                                        ...
                                        if (record.Balance >= stockNo * i.StockPrice)
                                        {
                                            if (stockNo > 0 && stockNo < i.StockVolume)
                                            {
                                                ...
                                                newObj.ShareValue = (stockNo * i.StockPrice);
                                                transaction.Transaction_List.Add(newObj);
                                                File.WriteAllText(@"G:\net\Stock_Data_Processing\Stock_Data_Processing\CustomerRecords.json", JsonConvert.SerializeObject(customer));
                                                File.WriteAllText(@"G:\net\Stock_Data_Processing\Stock_Data_Processing\StockAccount.json", JsonConvert.SerializeObject(stockAccount));
                                                File.WriteAllText(@"G:\net\Stock_Data_Processing\Stock_Data_Processing\Transaction.json", JsonConvert.SerializeObject(transaction));
                                                Console.WriteLine("\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t*****Customer data******" + "\n\t\t\tCustomer Id: " + record.Customer_Id + "\t\t\tUserName: " + record.UserName + "\n\t\t\tTotal_Shares:" + record.Total_Shares + "\t\t\tBalance:" + record.Balance);
                                                Console.Write("\t\t\tStock==>");
                                                foreach (KeyValuePair<string, int> val in record.StockDetails)
                                                {
